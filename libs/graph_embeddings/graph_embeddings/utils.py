...
def sample_center_context_pair(
    A, num_walks, walk_length, restart_prob, window_length, batch_size=100000
):
...
def generate_center_context_pair_from_config(trans_mat, gids, din, num_walks):
    num_nodes = len(gids)
    num_groups = int((np.max(gids) + 1))
    ...
    c2n = ((sparse.diags(din) @ U) @ np.diag(1.0 / np.maximum(1, Din))).T
    c2n = sparse.csr_matrix(c2n)

    c2n.data = _calc_cum_trans_prob(c2n.indptr, c2n.indices, c2n.data, num_groups)

    A = trans_mat.copy().astype(np.float)
    for i in range(num_groups):
        A[i, :] = np.cumsum(A[i, :]) / np.maximum(1e-8, np.sum(A[i, :]))

    return _generate_center_context_pair_from_config(
        A,
        gids.astype(int),
        c2n.indptr,
        c2n.indices,
...
def _generate_center_context_pair_from_config(
    trans_mat,
    gids,
    c2n_indptr,
    c2n_indices,
    c2n_data,  # should be cumulative
    num_groups,
    num_nodes,
    num_walks,
):
    pairs = np.zeros(num_nodes * num_walks).astype(np.int64)
    for it in prange(num_nodes * num_walks):
        ctr, walk_id = divmod(it, num_walks)
        next_gid = np.searchsorted(
            trans_mat[gids[ctr], :], np.random.rand(), side="right",
        )
        _next_node = np.searchsorted(
            trans_mat[gids[ctr], :], np.random.rand(), side="right",
        )
        cnt = c2n_indices[c2n_indptr[next_gid] + _next_node]

        pairs[it] = ctr + cnt  #
        pairs[it] = pairs[it] * (pairs[it] + 1) / 2 + cnt
...
def prep_sample_contetx_config(trans_mat, gids, din):
    num_nodes = len(gids)
    num_groups = int((np.max(gids) + 1))
...
def sample_context_config(
    center, trans_mat, gids, c2n_indptr, c2n_indices, c2n_data,  # should be cumulative
):
    context = np.zeros(len(center)).astype(np.int64)
    for it in prange(len(center)):
        ctr = center[it]
        next_gid = np.searchsorted(
            trans_mat[gids[ctr], :], np.random.rand(), side="right",
        )
        ...
        context[it] = c2n_indices[c2n_indptr[next_gid] + _next_node]
    return context
